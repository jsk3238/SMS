		...
		try :
			...
			for Server in self.ValueDict.keys() :
				for Type in self.ValueDict[Server].keys() :
					Msg = ''
					#Connected Fail
					if (Type == 'STATUS' and self.ValueDict[Server][Type] == 'NOK') : Msg = '[%s]\n%s\nConnected Fail' % (self.Title, Server)
					elif Type == 'IRIS' : 
						if self.ValueDict[Server][Type]['STATUS'] == 'NOK' : Msg = '[%s]\n%s\n%s\n%s' % (self.Title, Server, Type, '/'.join(self.ValueDict[Server][Type]['VALUE']))
					elif Type == 'MEMORY' or Type == 'SWAP' :
						if self.ValueDict[Server][Type]['STATUS'] == 'NOK' : Msg = '[%s]\n%s\n%s\n%s(%%)' % (self.Title, Server, Type, self.ValueDict[Server][Type]['VALUE'][IDX_MEMORY_USE_PER])
					elif Type == 'LOAD_AVG' :
						if self.ValueDict[Server][Type]['STATUS'] == 'NOK' : Msg = '[%s]\n%s\n%s\n%s' % (self.Title, Server, Type, '/'.join(self.ValueDict[Server][Type]['VALUE']))
					elif Type == 'DISK' :
						for Disk in self.ValueDict[Server][Type] :
							if Disk['STATUS'] == 'NOK' : Msg = '[%s]\n%s\n%s\n%s\n%s(%%)' % (self.Title, Server, Type, Disk['VALUE'][IDX_DISK_MOUNT], Disk['VALUE'][IDX_DISK_USE_PER])
					else : pass
			...
			for Corp in self.NumberDict.keys():			
				for Number in self.NumberDict[Corp] :
					for Msg in MsgList :
						sms_sock = socket.socket(socket.AF_INET, SOCK_DGRAM)
